...
On TUM, \oursFull~achieves a second-best ATE (3.15~cm) but underperforms \oursD2~alone on perceptual metrics (LPIPS 0.368 vs. 0.349).
...
\begin{tabular}{cccc}
  \includegraphics[height=3.8cm]{figures/rep_gt.png} &
  \includegraphics[height=3.8cm]{figures/rep_baseline.png} &
  \includegraphics[height=3.8cm]{figures/rep_replay.png} &
  \includegraphics[height=3.8cm]{figures/rep_full.png} \\\[2pt]
  \small GT &
  \small Baseline (36.6) &
  \small {+\oursD2~(38.5)} &
  \small \oursFull~(35.7) \\\
\end{tabular}
\caption{Rendering comparison on Replica room0 (training frame~5, single-frame PSNR in parentheses). \oursD2~achieves sharper reproduction of the dresser and window region (right side), consistent with reduced map drift in low-covisibility areas. \oursFull~exhibits a washed-out appearance near the lamp, corroborating the gradient-interference hypothesis.}
\label{fig:qualitative_rep}
\end{figure*}

\noindent\textbf{TUM fr3/office (Fig.~\ref{fig:qualitative_tum}).}
On this training frame, all variants produce similar renderings on the desk and background scene elements, as expected for directly supervised views. The key distinction is \oursFull: visible ghosting and blue-tinted smearing appear on the desk surface, consistent with gradient interference driving Gaussians to degenerate opacity configurations.

\noindent\textbf{Replica room0 (Fig.~\ref{fig:qualitative_rep}).}
The synthetic Replica scene provides a cleaner comparison. \oursD2~renders the dresser and window region with noticeably sharper detail (PSNR 38.5 vs.\ 36.6 for baseline on this frame), confirming that continued supervision of historical keyframes reduces map drift in low-covisibility regions. \oursFull~shows a washed-out appearance near the lamp (PSNR 35.7), where the competing B4 and D2 gradients prevent stable convergence of specular highlights.
