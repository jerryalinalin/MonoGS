...
Beyond {B4}~and {D2}, we explored four adaptive keyframe window strategies (A1--A4) and two adaptive densification schedules (C2--C3).
...
Moreover, the negative interaction with {B4}~warrants deeper investigation via gradient cosine similarity analysis or Hessian condition number estimation to characterize the loss landscape incompatibility.
...
We proposed view-direction-aware regularization ({B4}) and historical keyframe replay ({D2}).
Through rigorous ablation, we showed that {D2}~is a reliable, broadly applicable improvement, while {B4}~improves tracking at a perceptual cost, revealing a subtle trade-off in Gaussian geometry control.
Crucially, we demonstrated that the combination of {B4}~and {D2}~fails due to gradient interference, a negative result that is as instructive as the positive ones.
